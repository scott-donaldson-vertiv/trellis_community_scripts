...
echo -e "\n################################################################################"
echo -e "#"
echo -e "#\tVertiv Trellis(tm) Enterprise - Log Clear Aged Script $VERSION_STRING"
echo -e "#"
echo -e "################################################################################\n\n"
...
if [[ -d '/u02/domains/IDMDomain' ]]; then
  ...
  OUTPUT_LOG=`hostname -s`_trellis-back_logs-clear-aged_`date +"%Y%m%d-%H%M%S%z"`.log
  ...
  # TODO: Replace with filename function.
  OUTPUT_LOG=`hostname -s`_trellis-front_logs-clear-aged_`date +"%Y%m%d-%H%M%S%z"`.log
...
if [ $HOST == 'front' ]; then
	...
	for ((i=1; i<${wldomainlen}+1; i++)); do
		...
		find /u02/domains/$WL_DOMAIN/servers/${wlservers_front[$i-1]} -regextype posix-extended -regex '^.*(\.log|\.out|_log\.)[0-9]{0,12}' \
			-type f -ctime +${FILE_AGE_DAYS} \
			-exec rm -v {} \; 2>&1 | tee -a $OUTPUT_DIR/$OUTPUT_LOG
	done
	else
...
echo -e "[Info]: Logs saved to $OUTPUT_DIR/$OUTPUT_LOG" 2>&1 | tee -a $OUTPUT_DIR/$OUTPUT_LOG
echo -e "\nDone.\n" 2>&1 | tee -a $OUTPUT_DIR/$OUTPUT_LOG
